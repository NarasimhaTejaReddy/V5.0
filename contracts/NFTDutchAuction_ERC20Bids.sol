...
contract NFTDutchAuction_ERC20Bids is Initializable, UUPSUpgradeable {
    ...
    function bid() external payable returns (address) {
        ...
        uint256 currentPrice = initialPrice -offerPriceDecrement *(block.number - numBlocksAuctionOpen);
        require(myToken.balanceOf(msg.sender) >= currentPrice,"Your bid is lower than the minimum bid");
        auctionEnd = true;
